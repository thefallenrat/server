                     ...
                     DYNAMIC_ARRAY *keyuse_array)
{
  ...
  */
  ...
  for (s=stat ; s < stat_end ; s++)
  {
    ...
    if (s->table->s->tmp_table == INTERNAL_TMP_TABLE &&
        !s->table->is_splittable())
    {
      s->startup_cost= s->get_tmp_table_startup_cost(s->table->stat_records());
    }
...
double JOIN_TAB::get_tmp_table_startup_cost(ha_rows records)
{
  DBUG_ASSERT(table && (table->s->tmp_table == INTERNAL_TMP_TABLE));
  DBUG_ASSERT(join->best_read < DBL_MAX);
  double res= join->best_read;
  uint rec_size= table->s->rec_buff_length;
  res+= get_tmp_table_write_cost(join->thd, records, rec_size) * records;
  return res;
}
